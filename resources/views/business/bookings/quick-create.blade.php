...
@section('title', 'Quick Create Booking - The NexZen')
@section('page-title', 'Quick Create Booking')
...
<div class="row">
    <div class="col-lg-8">
        <div class="card">
            ...
            <div class="card-body">
                <form id="quickBookingForm" method="POST" action="{{ route('business.bookings.store') }}">
                    @csrf
                            ...
                            <select class="form-select" id="customer_id" name="customer_id" required onchange="loadCustomerDetails()">
                                ...
                                @foreach($customers as $customer)
                                    <option value="{{ $customer->id }}" 
                                            data-name="{{ $customer->full_name ?? $customer->company_name }}"
                            ...
                            <select class="form-select" id="vehicle_id" name="vehicle_id" required onchange="calculatePricing()">
                                ...
                                @foreach($vehicles as $vehicle)
                                    <option value="{{ $vehicle->id }}" 
                                            ...
                                            data-available="{{ $vehicle->is_available ? 'true' : 'false' }}"
                                            data-make="{{ $vehicle->vehicle_make }}"
                                            data-model="{{ $vehicle->vehicle_model }}"
                                            data-number="{{ $vehicle->vehicle_number }}">
                                        {{ $vehicle->vehicle_make }} {{ $vehicle->vehicle_model }} 
                                        ({{ $vehicle->vehicle_number }}) - ₹{{ number_format($vehicle->rental_price_24h ?? 1000) }}/day
    ...
    <div class="col-lg-4">
        ...
        <div class="card mt-3">
            ...
            <div class="card-body">
                <a href="{{ route('business.bookings.create') }}" class="btn btn-primary w-100 mb-2">
                    <i class="fas fa-magic me-2"></i>5-Step Booking Flow
                </a>
                <a href="{{ route('business.customers.create') }}" class="btn btn-outline-primary w-100 mb-2" target="_blank">
                    <i class="fas fa-user-plus me-2"></i>Add New Customer
                </a>
                <a href="{{ route('business.vehicles.create') }}" class="btn btn-outline-success w-100" target="_blank">
...
function calculatePricing() {
    ...
    fetch(`{{ url('business/bookings/calculate-pricing') }}`, {
        method: 'POST',
        headers: {
...
function submitForm() {
    ...
    .then(data => {
        if (data.success) {
            ...
            setTimeout(() => {
                window.location.href = '{{ route("business.bookings.index") }}';
            }, 1500);
        } else {
...
function openNewCustomerModal() {
    ...
    window.open('{{ route("business.customers.create") }}', '_blank');
}
