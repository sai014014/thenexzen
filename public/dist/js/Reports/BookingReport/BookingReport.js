...
async function triggerGlobalFilter() {
  try {
    ...
    // Fetch vehicle data
    await Promise.all([fetchBookingsList(queryParams)]);
...
// Fetch booking data from the server and update the DOM
async function fetchBookingsList(queryParams) {
  try {
    const response = await fetch(
      `booking-reportData?${queryParams.toString()}`,
      {
        method: "GET",
        headers: {
  ...
  if (response.data.length > 0) {
    ...
    response.data.forEach((vehicle) => {
      table.row.add([
        ...
        vehicle.Vehicles_Booked,
        vehicle.Unique_Customers,
        vehicle.Returning_Customers,
        vehicle.Cancelled_Bookings,
        numToFloat(vehicle.Cancellation_Rate) + "%",
        convertCurrencyFormat(vehicle.Total_Revenue),
        convertCurrencyFormat(vehicle.Average_Booking_Value),
      ]);
      srlno++;
    });
